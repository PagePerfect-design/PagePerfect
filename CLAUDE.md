...
- ~~**Quality systems advisory only:**~~ **PARTIALLY RESOLVED** — LaunchOverlay now shows a prominent quality warning when typography grade is C (amber) or D (red), with specific guidance to adjust margins/template/page size. Overfull line counts are surfaced. Grade D warns of "significant typography issues"; grade C warns that "adjusting margins or template may improve readability." Export is not hard-blocked (would frustrate users on free preview downloads) but the warning is prominent and must be scrolled past. TopBar also uses amber (not red) for soft/expired errors vs real compile failures.
