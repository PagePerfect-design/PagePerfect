      ...
      <div className="p-6 md:p-8">
        ...
        <div className="mb-8">
          ...
          <div className="mt-3 flex items-baseline gap-1.5">
            ...
            <span className="font-mono text-[11px] text-white/25">{tier.period}</span>
          </div>
          <p className="mt-3 font-body text-[15px] leading-relaxed text-white/35">{tier.desc}</p>
        </div>
        ...
        <ul className="mb-8 space-y-3">
          {tier.features.map((f) => (
            <li key={f} className="flex items-start gap-3 font-body text-[14px] text-white/40">
              <Check className={`mt-0.5 h-4 w-4 flex-shrink-0 ${tier.highlight ? 'text-[#0033ff]/60' : 'text-white/15'}`} />
              <span>{f}</span>
            </li>
          ))}
        </ul>
        ...
        <Link
          ...
          className={`group/btn flex h-12 w-full items-center justify-center gap-2 font-mono text-[12px] uppercase tracking-[0.1em] transition-all duration-200 ${
            tier.highlight
              ...
              : 'border border-white/[0.1] text-white/50 hover:border-white/[0.2] hover:text-white/70'
          }`}
        >
          {tier.cta}
