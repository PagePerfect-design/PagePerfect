...
export const ARTICLES_PART1: Article[] = [
  ...
  },
  {
    ...
    seo: {
      ...
      metaDescription: 'Greedy line-breaking creates rivers of white that disrupt saccades and destroy reading rhythm. Knuth-Plass and Typst micro-typography eliminate the problem.',
      primaryKeyword: 'rivers of white typography',
      secondaryKeywords: ['Knuth-Plass line-breaking algorithm', 'full justification readability', 'micro-typography glyph scaling', 'saccade disruption', 'ragged right alignment', 'Typst paragraph optimisation'],
      llmSummary: 'Analyses why word-processor full justification produces rivers of white — vertical cracks of negative space caused by greedy, line-at-a-time spacing algorithms that lack hyphenation dictionaries. Explains the physiological mechanism: irregular inter-word spacing disrupts saccadic eye movements, forcing conscious effort to track across lines and accelerating reader abandonment. Contrasts this with Knuth-Plass paragraph-level optimisation used by LaTeX, which distributes spacing tension across hyphenation, letter-spacing, and sub-perceptual glyph scaling. Recommends ragged right as the mandatory standard for any tool without algorithmic micro-typography. PagePerfect\'s Typst backend implements full Knuth-Plass-style paragraph optimization.',
      schemaType: 'TechArticle',
      proficiencyLevel: 'Advanced',
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          'We reject the notion that modern speed necessitates a decline in standards. PagePerfect utilizes Typst \u2014 a modern typesetting engine that inherits the algorithmic rigour of Donald Knuth\u2019s TeX while compiling in milliseconds, not minutes. While many modern word processors fail to provide the typographic flexibility required for serious publishing, our use of a programmatic backend ensures that every document follows the same rigorous rules once reserved for the master compositor.',
        ],
      },
    ],
    conclusion: {
  ...
  },
  {
    ...
    seo: {
      ...
      llmSummary: 'Applies software supply chain security principles to publishing infrastructure. Cites Chainguard research that 90% of application code is open-source dependencies, making supply chain integrity paramount. Details PagePerfect\'s Dockerised Typst pipeline: isolated compilation per job, deterministic build environments, and provenance metadata that encodes exact template, font, and engine versions for every export — a digital colophon enabling perfect reproducibility years later. Advocates SBOM implementation, hardened base images with near-zero CVEs, and auditable Coolify deployments. Frames operational integrity as a brand differentiator for creators whose intellectual property depends on platform trustworthiness.',
      schemaType: 'TechArticle',
      proficiencyLevel: 'Expert',
